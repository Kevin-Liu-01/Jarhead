...
Workers rail while the left two thirds show the apps being driven.
...
| 0:15 | "Tell Ben on Slack I'm running late, and put on Focus on Spotify." | "On it." Then **"Spotify alongside."** Spotify starts playing without the pointer moving. Slack comes to the front and the message is typed with Slack in front. The Workers rail shows `Spotify · working` and `Slack · working`. |
...
| 1:02 | "Open the Console." (or click the blob) | The Console: the conversation on the left, the Now stream with every step and screenshot, the Workers rail, the meter with dollars. Scroll once. |
...
- `pnpm jarhead status`: phase asleep, brain ready, hands ready, **16/16 permissions**, `workers 0`, no problems. If a problem row shows, fix it or the island will show it.
...
- Know the recovery moves: **Pause** (the island's dot) closes the paid session and holds the conversation; **Go** resumes; the Console's **Stop** on a worker row ends that worker only.
...
1. **The ledger.** Console › Ledger. Every row is append-only; show a `worker` row, the `sleep` row with its cause, a `grant` row from the yes. Search it.
...
- More than two workers. Two is the cap on purpose.

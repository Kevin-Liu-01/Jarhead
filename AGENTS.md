...
- **The harness is the eye.** `apps/mac/Scripts/console-preview.sh <scenario>` with `keyDown:down+return`, `click:(x,y)`, `focus:<id>`, `menuOpen:<id>`, `tipOpen:<id>`, `fold:<id>:<open|closed>`, `chip:<kind>`, `probe-floats`, and `check-kit@0.3` in every kit scenario; shoot dark and `PREVIEW_APPEARANCE=light`, once with `PREVIEW_REDUCE_MOTION=1`. Kit files live flat in `UI/Console/` (the harness globs `UI/Console/*.swift`); `UI/HelpCopy.swift` is AppKit-free and sits on `onboarding-preview.sh`'s swiftc line. The stream's rows have their own scenario, `resumed` (design13): a held session's rows before the live session's, `check-stream` (entry ids unique · caret on the newest utterance · the AX row texts in order == the snapshot's texts, read through the AX API over the harness's own pid), `republish` (the audio-state path) and `append`; it ends `check: all ok (stream)`.
